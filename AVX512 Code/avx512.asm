...
section .data
 ...
 global_count dq 0			; position count (checkpoint)
 mismatch_total dq 0        ; mismatches (checkpoint)
 checkpoint_offset dq 0     ; position in bytes (checkpoint)
 best_diagonal_score dq 0   ; best alignment per block (checkpoint)
...
Sneakynake:
	...
	; ===========================================
	; THIS IS FOR INITIALIZATION reset checkpoint system for this alignment
	xor rax, rax
    mov [global_count], rax         ; reset position counter
    mov [mismatch_total], rax       ; reset cumulative mismatches
    mov [checkpoint_offset], rax    ; reset byte offset
    mov rax, 64                     ; initialize to worst possible score
    mov [best_diagonal_score], rax	
	; ===========================================

	; compute byte length (divide nucleotides by 2 since 2 bases per byte)
    mov rdx, r11
    shr rdx, 1                      ; rdx = ReadLength / 2
    mov qword [read_bytes], rdx
    
    ; initialize loop variables
    xor r9, r9                      ; r9 = byte offset (starts at 0)
    xor r10, r10                    ; r10 = block mismatch counter
...
.mainloop:
	...
	; ===========================================
	; CHECKPOINT #1	- update position at start of block
	mov rax, r9                     ; current byte offset
    shl rax, 1                      ; convert to nucleotide count 
    mov [global_count], rax         ; store current position
    mov [checkpoint_offset], r9     ; store byte offset

	; reset block counters
	xor r10, r10                   ; reset block mismatch count
	mov rsi, 64                    ; initialize best score to worst 
	; ===========================================
...
.diagonals_done:
	...
	; ===========================================
	; CHECKPOINT #2	- after checking all diagonals, update cumulative total
	mov [best_diagonal_score], rsi    ; save best score found
	
	mov rax, [mismatch_total]         ; get current total
	add rax, rsi                      ; add this block's best score
	mov [mismatch_total], rax         ; store new total
	
	; this is for rejection check lng
	cmp rax, r14                      ; compare with threshold
	ja .rejected_early                ; if exceeded, exit immediately
	; ===========================================
...
.handle_tail:
	...
	; ===========================================
	; CHECKPOINT #3	- position for tail processing
	mov rax, r9
	shl rax, 1                        ; convert to nucleotides
	mov [global_count], rax
	; ===========================================
...
.tail_loop:
	...
	; ===========================================
	; CHECKPOINT #4	- tail mismatches to cumulative total
	; Add tail mismatches to cumulative total
	mov rax, [mismatch_total]
	add rax, r10                      ; add tail mismatches
	mov [mismatch_total], rax         ; store final total
	; ===========================================
...
.cleanup:
	...
	add rsp, 32			; restore shadow space - idt this is needed pa bc we're the callee?
