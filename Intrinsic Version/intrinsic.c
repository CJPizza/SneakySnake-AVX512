...
static inline int count_diagonal_matches_avx512(char* read, char* ref, int start, int end, int shift, int read_length, int is_right_diag) {
    ...
    if (is_right_diag) {
        // Right diagonal (deletion): read[i-shift] vs ref[i]
        for (; i <= end - 64; i += 64) {
            if (i - shift < 0) break;
        ...
        // Left diagonal (insertion): read[i+shift] vs ref[i]  
        for (; i <= end - 64; i += 64) {
...
int SneakySnake(int EditThreshold, char* ReadSeq, char* RefSeq, int ReadLength, int KmerSize, int IterationNo)
{
    int Edits = 0;
    
    if (KmerSize <= 0 || KmerSize > ReadLength) {
        KmerSize = 100;  // Default like original
    }
